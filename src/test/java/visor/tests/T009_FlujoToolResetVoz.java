...
public class T009_FlujoToolResetVoz {
    ...
    P005_ResetVozPage p005ResetVozPage = new P005_ResetVozPage();

    public String featureName;
    public String testername;

    public String sUrl, sBrowser, user, pass, escenario, tab, token;
    public String number, consultation, failure, tool;
    ...
    public Object[][]GetToolResetVoz() throws Exception {
        Object[][] testObjArray = null;
        testObjArray = ExcelUtils.getTableArray("./src/test/resources/data/TestData.xlsx", "ToolResetVoz");
        return (testObjArray);
    }
    ...
    @Given("^Tool Reset de voz - Se carga web Visor e inicio sesion como administrador \"([^\"]*)\"$")
    ...
    @When("^Busco el numero movil y selecciono card Averia \"([^\"]*)\"$")
    public void selecciono_card_averia(int id) throws Throwable {
        try {
            Object[][] Parameters = GetToolResetVoz();
    ...
    @And("^Doy click al card Herramientas y dar click a la pestaña VOZ \"([^\"]*)\"$")
    ...
    @Then("^Seleccionar card de Reset de voz y valido la respuesta obtenida para el tool \"([^\"]*)\"$")
    public void click_card_reset_voz_y_validar_respuesta(int id) throws Throwable {
        try {
            Object[][] Parameters = GetToolResetVoz();
            tool = Parameters[(id-1)][5].toString();
            p005ResetVozPage.voiceTool(id, tool);
